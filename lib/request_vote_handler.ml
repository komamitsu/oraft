...
let request_vote ~state ~logger ~cb_new_leader
  ...
  let persistent_log = state.persistent_log in
    ...
    then false
    else if PersistentState.detect_new_leader persistent_state ~logger ~other_term:param.term
    then (
...
let handle ~state ~logger ~cb_valid_request ~cb_new_leader
  ...
  then (
    cb_valid_request ();
    PersistentState.set_voted_for persistent_state ~logger ~voted_for:(Some param.candidate_id);
    Logger.debug logger
      (Printf.sprintf
